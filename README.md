...
- **消息**：`messages`（`content` 支持字符串或内容块数组；`image_url` 的 data-URL base64 图片会透传给视觉模型）
...
- **消息**：`system`（字符串或内容块数组）；`content` 支持 `text`、`image`（base64）、`tool_use`、`tool_result`、`thinking` 块
